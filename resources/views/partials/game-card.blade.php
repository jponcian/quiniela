    ...
    <div class="flex flex-col gap-6">
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <span class="font-bold text-lg text-white group-hover:text-brand-emerald transition-colors">{{ $game->team_a }}</span>
            </div>
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <span class="font-bold text-lg text-white group-hover:text-brand-emerald transition-colors">{{ $game->team_b }}</span>
            </div>
            @if($game->status == 'finished' || $game->status == 'in_play')
    ...
    @if(!$game->isLocked())
        @auth
            <button onclick="savePrediction({{ $game->id }})" id="btn_{{ $game->id }}"
                class="w-full mt-6 py-4 rounded-2xl bg-brand-emerald text-dark font-black text-xs uppercase tracking-widest transition-all hover:bg-brand-neon hover:scale-[1.02] active:scale-[0.98] shadow-lg shadow-brand-emerald/10">
                GUARDAR PRONÓSTICO
            </button>
        @else
            <a href="{{ url('/registro') }}" 
